...
async def wait_for_exit() -> None:
    ...
    if sys.platform != "win32":  # Unix / macOS
        ...
        # Let KeyboardInterrupt bubble up to asyncio.run to eliminate blocking caused by legacy threads during process exit
        try:
...
async def main():
    ...
    # auth_key is used for JWT authentication, such as vision analysis API JWT auth, OTA API token generation and websocket auth
    # Get auth_key from config file
    auth_key = config["server"].get("auth_key", "")
    
    # Validate auth_key, try manager-api.secret if invalid
    ...
    if not read_config_from_api:
        logger.bind(tag=TAG).info(
            "OTA API endpoint:\t\thttp://{}:{}/xiaozhi/ota/",
            get_local_ip(),
            port,
        )
    logger.bind(tag=TAG).info(
    ...
    if mcp_endpoint is not None and "你" not in mcp_endpoint:
        ...
        if validate_mcp_endpoint(mcp_endpoint):
            ...
            logger.bind(tag=TAG).error("MCP endpoint does not conform to specification")
            config["mcp_endpoint"] = "Your endpoint websocket address"

    # Get WebSocket config with safe defaults
    websocket_port = 8000
    ...
    logger.bind(tag=TAG).info(
        "=======The above address is a WebSocket protocol address, do not access via browser======="
    )
    logger.bind(tag=TAG).info(
        "To test WebSocket, open test_page.html in the test directory with Chrome browser"
    )
    logger.bind(tag=TAG).info(
        "=============================================================\n"
    )
