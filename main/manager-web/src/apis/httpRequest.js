...
function httpHandlerError(info, failCallback, networkFailCallback) {
    ...
    if (networkFailCallback) {
        ...
        showDanger(i18n.t('network.requestError', { status: info.status }))
    }
    return true
}
...
function reAjaxFun(fn) {
    ...
    if (ajaxIndex > 10) {
        showWarning(i18n.t('network.cannotConnect'))
    } else {
        showWarning(i18n.t('network.connecting', { index: ajaxIndex }))
    }
    if (ajaxIndex < 10 && fn) {
        setTimeout(() => {
            fn()
