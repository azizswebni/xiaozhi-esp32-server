...
CREATE TABLE `ai_voice_clone` (
    ...
    `user_id` BIGINT COMMENT 'User ID (references user table)',
    `voice` LONGBLOB COMMENT 'Voice data',
    `train_status` TINYINT(1) DEFAULT 0 COMMENT 'Training status: 0=pending, 1=training, 2=success, 3=failed',
